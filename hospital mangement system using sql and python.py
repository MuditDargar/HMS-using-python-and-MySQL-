...
def pd():
    if e1.get() == "" or e2.get() == "":
        ...
        con = mysql.connector.connect(
            host="localhost", username="root", password="Mudit@2002", database="mydata")
...
def get_data(event=''):
   ...
   #make the prescription Button Function
def pres():
      ...
      txt_frame.insert(END,'Patient Name:\t\t\t'+ patientid.get()+ '\n')
...
def delete():
   con = mysql.connector.connect(
       host="localhost", username="root", password="Mudit@2002", database="mydata")
   my_cursor = con.cursor()
   query = "DELETE FROM hospital WHERE `Name of Tablets` = %s"
...
# Heading
Label(win, text='Hospital Management System',
      font='impack 31 bold', bg='blue', fg='white').pack(fill=X)
# frame1
frame1 = Frame(win, bd=15, relief=RIDGE)
frame1.place(x=0, y=54, width=1280, height=310)
# Label frame for patient information
lf1 = LabelFrame(frame1, text='Patient Information',
                 font='ariel 10 bold', bd=10, bg='pink')
...
# Text variable flr Every entry field
...
txt_frame = Text(lf2, font='ariel 10 bold', width=40, height=30, bg='yellow')
...
# buttons
# delete buttons
d_btn = Button(win, text='Delete', font='arial 15 bold',
               bg='brown', fg='white', bd=6, cursor='hand2',command=delete)
d_btn.place(x=0, y=600, width=270)
# Prescription button
p_btn = Button(win, text='Prescription', font='arial 15 bold',
               bg='purple', fg='white', bd=6, cursor='hand2',command=pres)
p_btn.place(x=270, y=600, width=330)
# Save Prescription button
pd_btn = Button(win, text='Save Prescription Data', font='arial 15 bold',
                bg='green', fg='white', bd=6, cursor='hand2', command=pd)
pd_btn.place(x=600, y=600, width=340)
# clear button
c_btn = Button(win, text='Clear', font='arial 15 bold',
               bg='blue', fg='white', bd=6, cursor='hand2',command=clear)
c_btn.place(x=940, y=600, width=170)
# Exit  button
e_btn = Button(win, text='Exit', font='arial 15 bold',
               bg='red', fg='white', bd=6, cursor='hand2',command=exit)
...
table = ttk.Treeview(frame2, columns=('not', 'ref', 'dose', 'nots', 'issd', 'expd', 'dd',
                     'sd', 'pn', 'DOB', 'pa'), xscrollcommand=scroll_y.set, yscrollcommand=scroll_x.set)
